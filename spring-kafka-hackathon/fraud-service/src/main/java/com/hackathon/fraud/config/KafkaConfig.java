...
public class KafkaConfig {
    ...
    public ConsumerFactory<String, OrderCreatedEvent> consumerFactory() {
        ...
        props.put(ConsumerConfig.GROUP_ID_CONFIG, "fraud-service");
    ...
    @Bean
    public ProducerFactory<String, FraudEvent> producerFactory() {
        Map<String, Object> config = new HashMap<>();
        config.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, bootstrapServers);
        config.put(ProducerConfig.KEY_SERIALIZER_CLASS_CONFIG, StringSerializer.class);
        config.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, JsonSerializer.class);
        return new DefaultKafkaProducerFactory<>(config, new StringSerializer(),
                new JsonSerializer<FraudEvent>().noTypeInfo());
    }

    @Bean
    public KafkaTemplate<String, FraudEvent> fraudEventKafkaTemplate() {
